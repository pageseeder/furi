...
import org.pageseeder.furi.Variable.Reserved;
  ...
  public enum Syntax {
    ...
    DRAFT3 {
      ...
      protected Token newExpansion(String exp) {
        ...
        if (exp.indexOf('|') >= 0) return TokenOperatorD3.parse(exp);
    // assume a variable
    ...
    PAGESEEDER {
      ...
      protected Token newExpansion(String exp) {
        ...
        if (!Character.isLetter(exp.charAt(0)) && !Character.isDigit(exp.charAt(0))) return TokenOperatorPS.parse(exp);
    // assume a variable
        else return new TokenVariable(Variable.parse(exp));
      }
